...
class rescale:
    ...
    def _get_descale_args(W: int, H: int, width: Union[float, int], height: Union[float, int], base_height: int = None):
        if base_height is None:
            ...
            # if else by dtlnor patch about align odds number
            if base_height % 2 == 0:
                base_width = base_width // 2 * 2
            else:
                base_width = base_width // 2 * 2 + 1
            src_width = width
            src_height = height
    ...
    class Rescaler:
        def __init__(self, kernel: str = "bicubic", taps: int = 3, b: float = 0.0, c: float = 0.5, upscaler: Callable = None, fvfh: float = 1.0):
            kernel = kernel.lower()
            self.kernel = kernel
        ...
        def __call__(self, clip: vs.VideoNode, src_height: Union[int, float], upscaler: Optional[Callable] = None, src_blur: Optional[Union[int, float]] = None) -> Any:
        ...
        def descale(self, clip: vs.VideoNode, width: Union[int, float], height: Union[int, float], base_height: int = None, src_blur: Optional[Union[int, float]] = None):
            ...
            blur = 1/self.fvfh if src_blur is None else 1/src_blur
        ...
        def upscale(self, clip: vs.VideoNode, width: int, height: int, upscaler: Optional[Callable] = None, src_blur: Optional[Union[int, float]] = None) -> vs.VideoNode:
            ...
            kwargs.pop("height")
            if upscaler is None:
                return rescale.Upscale(clip, width, height, kernel=self.kernel, taps=self.taps, b=self.b, c=self.c, src_blur=self.fvfh if src_blur is None else src_blur, **kwargs)
            else:
...
def getnative(
    ...
    assert isinstance(ds_ref, vs.VideoNode) and clip.format.id == vs.GRAYS or ds_ref is None
    ...
    stats = stats_func(src_clip, rescaled) if invs_preflt is None else stats_func(src_clip, invs_preflt(rescaled))
